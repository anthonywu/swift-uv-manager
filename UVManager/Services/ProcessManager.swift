...
class ProcessManager: ObservableObject {
  ...
  func run(_ command: String, arguments: [String], streamOutput: Bool = false) async throws -> (
    ...
    return try await withCheckedThrowingContinuation { continuation in
      ...
      process.terminationHandler = { [weak self] _ in
        ...
        if process.terminationStatus == 0 {
          ...
          continuation.resume(
            throwing: ProcessError.nonZeroExit(code: process.terminationStatus, error: errorString))
        }
      }
